...
-- Tenant balance changes from payments are handled in Java (PaymentService), not triggers,
-- so only VERIFIED payments affect total_balance and status transitions are applied exactly once.
