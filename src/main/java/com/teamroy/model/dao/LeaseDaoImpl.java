...
public class LeaseDaoImpl implements LeaseDao {
    ...
    public void Create(Lease lease) {
        String sql = "INSERT INTO LEASE (tenant_id, room_id, start_date, end_date, monthly_rent, status, charged_rent_periods) VALUES (?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            ps.setInt(7, lease.GetChargedRentPeriods());
            ps.executeUpdate();
            ...
            // Shared Connection may have autoCommit=false (e.g. after RentAccrualService). Commit so the INSERT is durable.
    ...
    public void Update(Lease lease) {
        String sql = "UPDATE LEASE SET tenant_id=?, room_id=?, start_date=?, end_date=?, monthly_rent=?, status=?, charged_rent_periods=? WHERE lease_id=?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, lease.GetTenantID());
            ps.setInt(2, lease.GetRoomID());
            ps.setDate(3, Date.valueOf(lease.GetStartDate()));
    ...
    @Override
    public boolean UpdateChargedRentPeriods(int leaseId, int chargedRentPeriods) {
        String sql = "UPDATE LEASE SET charged_rent_periods = ? WHERE lease_id = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, chargedRentPeriods);
            ps.setInt(2, leaseId);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    @Override
    public boolean tryAdvanceChargedRentPeriods(int leaseId, int expectedCurrent, int newValue) {
        String sql = "UPDATE LEASE SET charged_rent_periods = ? WHERE lease_id = ? AND charged_rent_periods = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, newValue);
            ps.setInt(2, leaseId);
            ps.setInt(3, expectedCurrent);
    ...
    private Lease ResultSetToLease(ResultSet rs) throws SQLException {
        ...
        l.SetChargedRentPeriods(readChargedRentPeriods(rs));
        return l;
    }
    private static int readChargedRentPeriods(ResultSet rs) {
        try {
            int v = rs.getInt("charged_rent_periods");
