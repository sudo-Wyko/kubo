...
public class AddLeaseDialogController {
    ...
    private void save() {
        ...
        if (editingLease == null) {
            ...
            if (created == null) {
                if (!leaseDao.IsRoomAvailableWithCapacity(room.GetRoomID(), start, end, null, room.GetCapacity())) {
                    ...
                    showError("Could not save the lease to the database. Check that the LEASE table has a charged_rent_periods column "
                            + "and the application log for SQL errors.");
                }
                return;
            }
            notifySuccessClose();
