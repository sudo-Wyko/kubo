...
import com.teamroy.service.RentAccrualService;
...
public class AdminTenantController {
    ...
    private void reloadMainTable() {
        ...
        for (int i = 0; i < base.size(); i++) {
            Tenant t = base.get(i);
            int tid = t.GetTenantID();
            RentAccrualService.applyChargesThroughToday(conn, tid);
            Tenant refreshed = tenantDao.GetByID(tid);
            if (refreshed != null) {
                base.set(i, refreshed);
            }
        }
    ...
    private void onTenantSelected(Tenant tenant) {
        ...
        LocalDate nextDue = RentAccrualService.nextDueDate(lease);
        if (nextDue == null || nextDue.isAfter(lease.GetEndDate())) {
            detailNextDue.setText("Next due: \u2014");
        } else {
